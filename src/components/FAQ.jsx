...
const FAQ = () => {
  ...
  return (
    <div id='faq' className='px-[2rem] md:px-[3rem] lg:px-[12rem] lg:mt-[13rem] mt-[5rem] flex flex-col font-poppins'>
      <h2 className='lg:text-[40px] md:text-[35px] text-[30px] text-white font-bold text-center font-mPlus1 mb-[2rem] lg:mb-[2rem]'>FAQ<span className='lg:text-[30px] text-[23px]'>S</span></h2>

      <div>
      {accordion.map((list, index) => (
        <Accordion className='mb-[15px]' key={index} open={open === index} animate={CUSTOM_ANIMATION} icon={open ? <FaMinus /> : <FaPlus />}>
          <AccordionHeader className='text-white lg:text-[22px] md:text-[20px] text-[16px] font-bold leading-6 hover:text-[#2BB32A]' onClick={() => handleOpen(index)}> {list.question} </AccordionHeader>
          <AccordionBody className="text-[#ADB2B1] lg:text-[16px] md:text-[14px] text-[12px] leading-6 "> {list.answer} </AccordionBody>
        </Accordion>
      ))}
